...
{
	return ALP_TSANITY_VALID(q, &q->v[0], ALP_NULL);
}
...
{
	if ( ALP_SANITY_VALID(qIn && qOut) && qIn != qOut )
	{
...
{
	if ( ALP_SANITY_VALID(qLHS && qRHS && qOut) )
	{
		qOut->v[ALQM_QUATX] =
...
{
	return ALP_TSANITY_VALID(q0 && q1,
...
{
	return ALP_TSANITY_VALID(qLHS && qRHS,
